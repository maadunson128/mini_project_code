...
// Global variable to hold the last used ID
int lastID = 0;
...
void fetchLastID() {
    if (Firebase.RTDB.getInt(&fbdo, "/lastID")) {
        lastID = fbdo.intData(); // Correct method to get integer data
        Serial.println("Last ID retrieved: " + String(lastID));
    } else {
        Serial.println("Failed to get last ID: " + fbdo.errorReason());
        lastID = 0; // Default to 0 if retrieval fails
    }
}


// Function to update the last used ID in Firebase
void updateLastID() {
    lastID++; // Increment ID for the new entry
    Firebase.RTDB.setInt(&fbdo, "/lastID", lastID); // Save the new ID to Firebase
}
...
void calculateAndLogData() {
    ...
    // Define Firebase path using lastID as the unique ID
    String path = "/waterData/" + String(lastID);

    // Push JSON object to Firebase
    if (Firebase.RTDB.setJSON(&fbdo, path.c_str(), &json)) {
        Serial.println("Data logged successfully with ID: " + String(lastID));
        updateLastID(); // Update last used ID after logging data
    } else {
...
void setup() {
    ...
    fetchLastID(); // Fetch the last used ID from Firebase
    setupNTP();  // Setup NTP for time synchronization
}

void loop() {
    calculateAndLogData();  // Calculate and log data to Firebase
    delay(15000);  // Wait for 15 minutes (900000 ms) before the next reading
}
